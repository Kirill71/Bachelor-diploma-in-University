...
{
    return std::make_unique< MatrixImpl >( _rowSize );

} // Matrix::makeMatrixImpl

/*---------------------------------------------------------------------------*/

Matrix::Matrix( const size_t _rowSize, const size_t _collSize )
    :	m_rowSize( _rowSize )
...
operator* ( const Matrix::Row & _row, const Matrix::MatrixDataType _scalar )
{
    ...
    std::transform(
        ...
        ,   _row.end()
        ,   result.begin()
        ,   std::bind( std::multiplies< >(), _1, _scalar )
    );
        ...
        ,   [ & ]( const Matrix::Row& _row )
            {
                std::for_each(
                        _row.cbegin()
                    ,   _row.cend()
                    ,   [ & ]( const Matrix::MatrixDataType _value )
                        {
                            _ostream << _value << " ";
                        }
                );
...
const std::istream&
operator>> ( std::istream & _istream, const Matrix & _matrix )
{
    std::for_each(
            _matrix.cbegin()
        ,   _matrix.cend()
